        ...
        def _update_step(update_runner_state, unused):
            ...
            def _calculate_gae(traj_batch, last_val, shaped_reward_coeff):
                def _get_advantages(gae_and_next_value, transition):
                    ...
                    shaped_reward = transition.reward + scaled_shaped_reward # log in wandb
                    done, value, reward = (
                        transition.done,
                        transition.value,
                        shaped_reward,
            ...
            def callback(metric): #State, traj_batch):# , advantages, targets, total_loss, value_loss, loss_actor, entropy):
                wandb.log({ 
                    "total_rewards": shapedRewardState.shaped_reward_coeff*traj_batch.shaped_reward.sum(axis=0).mean(axis=-1) + traj_batch.reward.sum(axis=0).mean(axis=-1), # [jnp.array([13, 27, 41, 55,69,83,97,111]), :].mean(), 
                    "total_rewards_per_game_estimate": (shapedRewardState.shaped_reward_coeff*traj_batch.shaped_reward.sum(axis=0).mean(axis=-1) + traj_batch.reward.sum(axis=0).mean(axis=-1)) / (traj_batch.reward.shape[0]/14), 
                    "env_step": metric["update_steps"],# 
                    # * config["NUM_ENVS"] 
                    # * config["NUM_STEPS"],
                    "shaped_coefficient": shapedRewardState.shaped_reward_coeff, 
                    "scaled_shaped_reward": shapedRewardState.shaped_reward_coeff*traj_batch.shaped_reward.sum(axis=0).mean(), 
            ...
            io_callback(callback, None, metric) #, shapedRewardState, traj_batch) # , traj_batch, advantages, targets, total_loss, value_loss, loss_actor, entropy)
