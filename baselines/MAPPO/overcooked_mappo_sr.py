...
def get_rollout(train_state, config, layout):
    # env = jaxmarl.make(config["ENV_NAME"], **config["ENV_KWARGS"]) # NOTE
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                transition = Transition(
                    ...
                    info["shaped_reward"], # already batchified?
                    log_prob,
                    obs_batch,
                    world_state_batch,
                    info
            ...
            def callback(metric, train_state):#, traj_batch, advantages, targets, total_loss, value_loss, loss_actor, entropy):
                ...
                train_state = train_state.set_new_shaped_reward_coeff(new_coeff=new_shaped_reward_coeff)
...
#     filename = f'{config["ENV_NAME"]}_cramped_room'(5, 610, 128, 256)
#     plt.ylabel("Return")
...
def main(config):
    ...
    for layout in config["LAYOUTS"]:
        ...
        filename = f'mappo_{config["ENV_NAME"]}_{layout}_sr'
