...
class RSSM(nj.Module):
  ...
  def observe_nonrecurrent_enc(self, embed, action, is_first, state=None):
    ...
    post_stoch = jnp.concatenate([state['stoch'], post['stoch']], axis=1)
    # post_stoch = jnp.concatenate([
    #   state['stoch'][:, None], post['stoch'][:, :-1]], 1)

    if self._classes:
      shape = post_stoch.shape[:-2] + (2 * self._stoch * self._classes,)
  ...
  # def observe_nonrecurrent_enc(self, embed, action, state=None):
  #   """
  #   action.shape (b, t, adim)
  #   is_first.shape (b, t,)
  #   state { t=-1
  ...
  def obs_step(self, prev_state, prev_action, embed, is_first):
    ...
    # if self._nonrecurrent_enc: NOTE: hier nochmal schauen wegen conc..
    #   x = embed
    # else:
    # x = jnp.concatenate([prior['deter'], embed], -1)
    """
    shape von deter anschauen, wieso 256? 128?, embed um eine dim erweitern -> conc
    (Pdb) embed
    Traced<ShapedArray(float32[128])>with<DynamicJaxprTrace(level=1/0)>
    (Pdb) prior['deter']
    Traced<ShapedArray(float32[256,4096])>with<DynamicJaxprTrace(level=1/0)>
    """
    # x = jnp.concatenate([prior['deter'], embed], -1) 
    # try:
    #   x = jnp.concatenate([prior['deter'], embed], axis=-1) # NOTE: hier nochmal anschauen..
    # except TypeError:
    #   x = embed # NOTE: fix this..
    # x = embed
    x = prior['deter'] # NOTE: why do prior['stoch'] prior['logits'] get right shape here but not when conc with embed?
  ...
  def img_step(self, prev_state, prev_action):
    ...
    if len(prev_action.shape) == 1:
      prev_action = jnp.expand_dims(prev_action, -1)
    x = jnp.concatenate([prev_stoch, prev_action], -1)
    x = self.get('img_in', Linear, **self._kw)(x)
    x, outs = self._cell(x, prev_state) # ssm_nets/_cell
    x = self.get('img_out', Linear, **self._kw)(x)
    stats = self._stats('img_stats', x)
    dist = self.get_dist(stats)
    stoch = dist.sample(seed=nj.rng())
  ...
  def initial(self, bs):
    if self._classes:
      state = dict(
          ...
          hidden=jnp.zeros([bs, self._deter], jnp.float32),
    ...
    return cast(state)
    # NOTE: Investiage methods below: Need? Just init deter, stoch if not available?
